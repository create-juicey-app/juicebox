...
use crate::util::{now_secs, ttl_to_duration, ADMIN_SESSION_TTL, ADMIN_KEY_TTL, new_id};

#[derive(Serialize, Deserialize, Clone, Debug)] pub struct FileMeta { pub owner: String, pub expires: u64, #[serde(default)] pub original: String }
...
impl AppState {
    ...
    pub async fn load_or_create_admin_key(&self, path: &PathBuf) -> anyhow::Result<AdminKeyFile> {
        ...
        let new = AdminKeyFile { key: new_id(), expires: now_secs() + ADMIN_KEY_TTL };
