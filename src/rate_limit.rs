use std::{collections::HashMap, sync::Arc, task::{Context, Poll}, future::Future, pin::Pin, time::Instant};
...
#[derive(Clone, Debug)] struct RateBucket { tokens: f64, last: Instant }

#[derive(Clone)] pub struct RateLimiterInner { buckets: Arc<RwLock<HashMap<String, RateBucket>>>, cfg: Arc<RateLimitConfig> }
impl RateLimiterInner { pub fn new(capacity: u32, refill_per_second: u32) -> Self { Self { buckets: Arc::new(RwLock::new(HashMap::new())), cfg: Arc::new(RateLimitConfig{capacity, refill_per_second}) } }
    pub async fn check(&self, ip: &str) -> bool { let mut map=self.buckets.write().await; let entry=map.entry(ip.to_string()).or_insert(RateBucket{tokens:self.cfg.capacity as f64,last:Instant::now()}); let now=Instant::now(); let elapsed=now.duration_since(entry.last).as_secs_f64(); if elapsed>0.0 { let refill=elapsed*self.cfg.refill_per_second as f64; entry.tokens=(entry.tokens+refill).min(self.cfg.capacity as f64); entry.last=now; } if entry.tokens>=1.0 { entry.tokens-=1.0; true } else { false } }
}
...
where S: Service<Request<Body>, Response = Response> + Clone + Send + 'static,
      S::Error: std::fmt::Display,
      S::Future: Send + 'static {
    type Response = S::Response; type Error = S::Error; type Future = Pin<Box<dyn Future<Output=Result<Self::Response, Self::Error>> + Send>>;
...
pub fn build_rate_limiter() -> RateLimitLayer { RateLimitLayer::new(60,1) }
