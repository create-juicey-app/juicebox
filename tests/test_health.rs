...
async fn test_health_check_wrong_method() {
    ...
    assert!(response.status() == StatusCode::METHOD_NOT_ALLOWED || response.status() == StatusCode::OK);
}
...
async fn test_health_check_with_headers() {
    ...
    let app = build_router(state);
    let response = app
        .oneshot(Request::builder().uri("/healthz").header("X-Test", "1").body(Body::empty()).unwrap())
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
