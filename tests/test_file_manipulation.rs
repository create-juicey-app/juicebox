...
async fn test_list_multiple_files() {
    ...
    let text = String::from_utf8(list_body.to_vec()).unwrap();
    assert!(
        text.contains("multi0.txt") && text.contains("multi1.txt") && text.contains("multi2.txt")
    );
}
