...
async fn test_upload_multiple_files_limit() {
    ...
    for i in 0..6 {
        ...
        let upload = with_conn_ip(Request::builder().method(Method::POST).uri("/upload").header(header::CONTENT_TYPE, ct).body(body).unwrap(), [10,10,10,10], 9000);
        let resp = app.clone().oneshot(upload).await.unwrap();
        if i == 0 {
            ...
            assert!(resp.status() == StatusCode::CONFLICT || resp.status() == StatusCode::BAD_REQUEST, "Expected 409 or 400 for upload at or beyond limit");
        }
    }
}
...
async fn test_upload_file_at_max_size() {
    ...
    let upload = with_conn_ip(Request::builder().method(Method::POST).uri("/upload").header(header::CONTENT_TYPE, ct).body(body).unwrap(), [11,11,11,11], 1111);
    let resp = app.clone().oneshot(upload).await.unwrap();
    assert!(resp.status() == StatusCode::OK || resp.status() == StatusCode::BAD_REQUEST);
}
...
async fn test_forbidden_extension_uppercase() {
    ...
    let upload = with_conn_ip(Request::builder().method(Method::POST).uri("/upload").header(header::CONTENT_TYPE, ct).body(body).unwrap(), [12,12,12,12], 1212);
    let resp = app.clone().oneshot(upload).await.unwrap();
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
}
