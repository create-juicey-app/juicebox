...
async fn test_report_missing_details() {
    ...
    let upload = with_conn_ip(Request::builder().method(Method::POST).uri("/upload").header(header::CONTENT_TYPE, ct).body(body).unwrap(), [192,168,0,2], 3002);
    let resp = app.clone().oneshot(upload).await.unwrap();
    ...
    let report_req = with_conn_ip(Request::builder().method(Method::POST).uri("/report").header(header::CONTENT_TYPE, "application/x-www-form-urlencoded").body(Body::from(form)).unwrap(), [192,168,0,2], 3003);
    let report_resp = app.clone().oneshot(report_req).await.unwrap();
    // Should be accepted or rejected gracefully
    assert!(report_resp.status() == StatusCode::NO_CONTENT || report_resp.status() == StatusCode::BAD_REQUEST);
}
...
async fn test_report_invalid_file() {
    ...
    let report_req = with_conn_ip(Request::builder().method(Method::POST).uri("/report").header(header::CONTENT_TYPE, "application/x-www-form-urlencoded").body(Body::from(form)).unwrap(), [192,168,0,3], 3004);
    let report_resp = app.clone().oneshot(report_req).await.unwrap();
    assert!(report_resp.status() == StatusCode::BAD_REQUEST || report_resp.status() == StatusCode::NO_CONTENT);
}
...
async fn test_report_empty_reason() {
    ...
    let upload = with_conn_ip(Request::builder().method(Method::POST).uri("/upload").header(header::CONTENT_TYPE, ct).body(body).unwrap(), [192,168,0,4], 3005);
    let resp = app.clone().oneshot(upload).await.unwrap();
    if resp.status() != StatusCode::OK { return; }
    let body_bytes = to_bytes(resp.into_body(), usize::MAX).await.unwrap();
    let up: UploadResponse = serde_json::from_slice(&body_bytes).unwrap();
    ...
    let report_req = with_conn_ip(Request::builder().method(Method::POST).uri("/report").header(header::CONTENT_TYPE, "application/x-www-form-urlencoded").body(Body::from(form)).unwrap(), [192,168,0,4], 3006);
    let report_resp = app.clone().oneshot(report_req).await.unwrap();
    assert!(report_resp.status() == StatusCode::NO_CONTENT || report_resp.status() == StatusCode::BAD_REQUEST);
}
